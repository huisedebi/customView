package com.xinyartech.baselibrary.view.flowtag;
...
//              new MarginLayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));//这个构造方法所然能使用但是编译器会报错
            ...
            if (mAdapter instanceof OnInitSelectedPosition) {
                ...
                LogUtil.LogShitou("FlowTagLayout--reloadData", i+"   "+isSelected);
                //判断一下模式
                if (mTagCheckMode == FLOW_TAG_CHECKED_SINGLE) {
                    ...
                    if (isSelected) {
                        LogUtil.LogShitou("FlowTagLayout--reloadData", ""+i);
                        mCheckedTagArray.put(i, true);
                        childView.setSelected(true);
                    }
                }
...
//                        }
                        ...
                        LogUtil.LogShitou("FlowTagLayout--onClick", ""+mCheckedTagArray.get(j));
                        if (mCheckedTagArray.get(j)) {
                            mCheckedTagArray.put(j, false);
                            childView.setSelected(false);
                        } else {
...
//            getChildAt(selectList.get(i)).setSelected(true);
//        }
    }
